...
class PlatformNavigationBarCloseButton
    extends PlatformWidgetBase<Widget, Widget, Widget> {
  const PlatformNavigationBarCloseButton({
    super.key,
    this.buttonText,
    required this.onPressed,
...
class PlatformNavigationBarBackButton extends StatelessWidget {
  const PlatformNavigationBarBackButton({
    super.key,
    this.buttonText,
    required this.onPressed,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
      Path.combine(
        ...
        Path()
          ..addRRect(RRect.fromLTRBR(0, 0, 60, 60, const Radius.circular(0))),
        Path()
          ..addOval(Rect.fromCircle(center: const Offset(28, 28), radius: 22))
          ..close(),
      ),
      paint,
    );
